 ...
 **/
  ...
  **/
  var addSelect = function (id, label, items, isMultiple = 0, def = "", name_tag="name") {
    fields.push({ "type": TYPE_SELECT, "id": id, "label": label, "items": items, "default": def, "isMultiple": isMultiple, "name_tag":name_tag, "classes": "" });
    return this;
  }
  ...
  */
  ...
  var selectTemplate = function (field) {
    ...
    for (let item of field.items) {
      if (field.default == item.id) {
        content += '<option value="' + item.id + '" selected>' + item[field.name_tag] + '</option>';
      } else {
        content += '<option value="' + item.id + '" >' + item[field.name_tag] + '</option>';
      }
    }
  ...
  var locationMapTemplate = function (field) {
    ...
    return '<label class="control-label ' + field.classes + '" for="map_canvas' + field.latid + '">' + field.label + '</label>' +
      ...
      '<p>Latitude: <input class="form-control"  size="20" type="text" id="map' + field.latid + '" name="gpslat"  value="' + field.latdef + '" ></p>' +
      '<p>Longitude: <input class="form-control"  size="20" type="text" id="map' + field.lngid + '" name="gpslng" value="' + field.lngdef + '"  ></p>' +
      '</div>' +
      '</div>';
  }
